...
if ($listeners) {
    ...
    foreach ($l in $listeners) {
        try {
            ...
            Write-Host (" PID {0}: {1}" -f $l.OwningProcess, $p.ProcessName) -ForegroundColor Yellow
        } catch {}
    }
} else {
...
Write-Host "`nRun this script like: .\tools\server_diagnose_fixed.ps1 -Port 5000" -ForegroundColor Cyan
